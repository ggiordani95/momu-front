...
import { SocialWorkspace } from "@/modules/social";
import { AIWorkspace } from "@/modules/ai";
...
function HomeContent({
  ...
  // Map pendingItems (which uses "item") to the shape expected by ExplorerWorkspace ("file")
  const pendingFiles = useMemo(() => {
    const map = new Map<
      string,
      { file: HierarchicalFile; data: CreateFileDto }
    >();
    pendingItems.forEach(({ item, data }, key) => {
      map.set(key, { file: item, data });
    });
    return map;
  }, [pendingItemsKey, pendingItems]);
