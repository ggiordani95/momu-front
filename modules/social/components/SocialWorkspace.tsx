...
export function SocialWorkspace() {
  ...
  const columns = [
    ...
    },
    {
      id: "draft",
      title: "Rascunho",
      icon: <FileTextIcon size={16} className="text-blue-500" />,
    },
    {
      id: "scheduled",
      title: "Agendado",
    ...
    <div className="h-full flex flex-col p-8 overflow-y-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-foreground/60">
          Gerencie sua presença digital e planeje seu conteúdo.
        </p>
      </div>
              ...
              <div className="flex-1 bg-foreground/5 rounded-xl p-3 space-y-3">
                {posts
                  ...
                  .map((post) => (
                    <div
                      key={post.id}
                      className="bg-background border border-border/50 p-4 rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer group"
                    >
...
function AccountCard({
  ...
  return (
    <div
      className={`flex items-center gap-4 p-4 rounded-xl border transition-all w-64 ${
        connected
          ? "bg-background border-border/50"
          : "bg-foreground/5 border-transparent opacity-70"
      }`}
    >
      <div className="p-2.5 bg-foreground/5 rounded-lg">{icon}</div>
      <div className="flex-1">
        <h3 className="font-medium text-sm">{name}</h3>
        <p className="text-xs text-foreground/60">
          {connected ? handle : "Não conectado"}
        </p>
      </div>
      <div
